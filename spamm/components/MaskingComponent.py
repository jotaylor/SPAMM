...
def defaultreadmaskregions(maskType=None,mask_FWHM_broad=5000,mask_FWHM_narrow=1000,wavelengths=None,flux=None,flux_err=None):
        ...
        absorption_list = []
        dlamda = wavelengths[1]-wavelengths[0]
        absorption = Determine_absorption_features(wavelengths,flux/np.nanmedian(flux),flux_err/np.nanmedian(flux),1180.)
        ...
        if maskType =="Absorption":
            return absorption_list



def Mask(wavelengths=None,flux=None, flux_err=None, maskType=None,mask_FWHM_broad=5000,mask_FWHM_narrow=1000):
            _mask = [False]*len(wavelengths)
            if maskType != None:
                wavelength = np.array(wavelengths)
                wavebound = defaultreadmaskregions(maskType=maskType,mask_FWHM_broad=mask_FWHM_broad,mask_FWHM_narrow=mask_FWHM_narrow,wavelengths=wavelengths,flux=flux,flux_err=flux_err)
                for k in range(len(wavebound)):
                    select = np.nonzero((wavelength >= wavebound[k][0]) & (wavelength <= wavebound[k][1]))
                    for x in select[0]:
                        _mask[x] = True
                if (maskType == "Continuum") or (maskType == "FeRegions") or (maskType == "Cont+Fe"):
