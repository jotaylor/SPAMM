...
class FeComponent(Component):
	...
	This component has 2 kinds of parameter:
	
	parameter1 : width of broadening kernal
	parameter2 : template normalization for each template
...
# 	        self.template_flux_model_grid = np.array(self.template_flux_model_grid)
	...
	def flux(self, spectrum=None, parameters=None):
		...
		for i in range(len(self.templates)):
			# Remember that the normalization below changes the final values compared
			#   to what you might expect from the sim data.
